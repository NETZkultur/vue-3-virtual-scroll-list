import { defineComponent, getCurrentInstance, ref, computed, onBeforeUpdate, h, nextTick } from 'vue';
...
var script = defineComponent({
  ...
  setup(props, context) {
    ...
    const getVarSize = (index, nocache) => {
    ...
    const getVarOffset = (index, nocache) => {
      const cache = delta.varCache[index];
    ...
    onBeforeUpdate(() => {
      ...
      const zone = getZone(calcstart); // if start, size or offset change, update scroll position.

      if (changeProp && ["start", "size", "offset"].includes(changeProp)) {
        const scrollTop = changeProp === "offset" ? props.offset : props.variable ? getVarOffset(zone.isLast ? delta.total : zone.start) : zone.isLast && delta.total - calcstart <= props.remain ? delta.total * props.size : calcstart * props.size;
    ...
    const updateZone = offset => {
      let overs = props.variable ? getVarOvers(offset) : Math.floor(offset / props.size); // if scroll up, we'd better decrease it's numbers.
      ...
      const bench = props.bench || props.remain; // for better performance, if scroll passes items within the bench, do not update.
